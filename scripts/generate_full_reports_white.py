...
OUTPUT_DIR = os.path.join(BASE_DIR, "relatorios-apm-completos-white")
...
.cover {{
    ...
    background: #FFFFFF;
    color: #1A202C; display: flex; flex-direction: column;
    justify-content: center; padding: 50mm 30mm;
    page-break-after: always; position: relative; overflow: hidden;
}}
.cover::before {{
    content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0;
    background: repeating-conic-gradient(rgba(13,115,119,0.02) 0% 25%, transparent 0% 50%) 0 0 / 30px 30px;
    pointer-events: none;
}}
.cover .brand {{
    display: flex; align-items: center; gap: 5mm; margin-bottom: 20mm; position: relative;
}}
.cover .brand .apm-img {{ height: 18mm; }}
.cover .brand .partner-text {{ font-size: 9pt; color: #718096; }}
.cover .brand .partner-text b {{ display: block; font-size: 10pt; color: #0A5C5F; }}
.cover .subtitle {{ font-size: 11pt; color: #718096; margin-bottom: 25mm; font-weight: 300; position: relative; }}
.cover h1 {{ font-family: 'Source Serif 4', serif; font-size: 30pt; font-weight: 700; margin-bottom: 6mm; line-height: 1.15; position: relative; color: #0A5C5F; }}
.cover .sub2 {{ font-size: 11pt; color: #718096; margin-bottom: 25mm; font-weight: 300; position: relative; }}
.cover .big-number {{ font-family: 'Inter', sans-serif; font-size: 48pt; font-weight: 800; color: #0D7377; margin-bottom: 3mm; position: relative; }}
.cover .big-label {{ font-family: 'Source Serif 4', serif; font-size: 13pt; color: #4A5568; margin-bottom: 12mm; position: relative; }}
.cover .pct-badge {{
    display: inline-block; border: 2px solid #0D7377; background: rgba(13,115,119,0.06);
    padding: 3mm 8mm; border-radius: 4mm; font-size: 16pt; font-weight: 700; color: #0D7377;
    font-family: 'Inter', sans-serif; position: relative;
}}
.cover .footer {{ margin-top: auto; font-size: 8.5pt; color: #A0AEC0; position: relative; }}
.cover .footer b {{ color: #0D7377; }}
.cover .cover-line {{
    width: 60mm; height: 1mm; background: linear-gradient(90deg, #0D7377, #11998E);
    border-radius: 0.5mm; margin-bottom: 8mm; position: relative;
}}
